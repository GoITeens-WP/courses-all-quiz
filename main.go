...
func main() {
	// Чтение JSON файла
	data, err := os.ReadFile("create-starter.json")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	newHTML := strings.Replace(string(indexHTML), "<!-- import html partials -->", insert, 1)
	...
	newSCSS := strings.Replace(string(mainSCSS), "/* <!-- import styles --> */", insertSCSS, 1)
