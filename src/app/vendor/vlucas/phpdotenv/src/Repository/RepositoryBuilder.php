...
final class RepositoryBuilder
{
  ...
  {
    if (
      !(\is_string($reader) && self::isAnAdapterClass($reader)) &&
  ...
  {
    if (
      !(\is_string($writer) && self::isAnAdapterClass($writer)) &&
      !($writer instanceof WriterInterface)
    ) {
      throw new InvalidArgumentException(
        \sprintf(
          'Expected either an instance of %s or a class-string implementing %s',
  ...
  {
    if (
      !(\is_string($adapter) && self::isAnAdapterClass($adapter)) &&
