...
final class EntryParser
{
  ...
  private static function parseName(string $name)
  {
    if (
      Str::len($name) > 8 &&
      Str::substr($name, 0, 6) === 'export' &&
      \ctype_space(Str::substr($name, 6, 1))
    ) {
  ...
  {
    return Regex::matches('~\A[a-zA-Z0-9_.]+\z~', $name)
      ->success()
      ->getOrElse(false);
  }
  ...
  private static function parseValue(string $value)
  {
    ...
    return \array_reduce(
      \iterator_to_array(Lexer::lex($value)),
      static function (Result $data, string $token) {
        return $data->flatMap(static function (array $data) use ($token) {
          return self::processToken($data[1], $token)->map(static function (array $val) use (
            $data
          ) {
      ...
      ->flatMap(static function (array $result) {
        ...
        if ($token === '"' || $token === '\\') {
          ...
          if (\in_array($first, ['f', 'n', 'r', 't', 'v'], true)) {
            ...
            return Success::create([
              \stripcslashes('\\' . $first) . Str::substr($token, 1),
              false,
              self::DOUBLE_QUOTED_STATE,
            ]);
          } else {
